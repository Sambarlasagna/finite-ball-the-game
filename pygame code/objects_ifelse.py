            ...
            if event.type == pygame.KEYUP or event.type == pygame.KEYDOWN:
                ...
                try:
                    ...
                    if event.type == pygame.KEYUP:
                        #if key is release
                        try:
                            #remove the key from set which is release
                            self.pressed.remove(key)
                        except KeyError as e:
                            pass
                    else:
                        #if key is pressed
                        #add presently pressed keys in set()
                        self.pressed.add(key)
    
                    # Reset horizontal and vertical movement
                    self.S0()
                    
                    # Determine which keys are currently held down
                    is_right = False
                    is_left = False
                    is_up = False
                    is_down = False
                    ...
                    # Apply logic with if-else statements instead of DFA
                    if is_right:
                        self.S1()
                    elif is_left:
                        self.S2()
